...
class DT_Metrics_Critical_Path_Charts_location extends DT_Metrics_Chart_Base
{
    ...
    public $base_title;
    public $title;
    public $slug = 'critical_path_location'; // lowercase
    public $js_object_name = 'wp_js_object'; // This object will be loaded into the metrics.js file by the wp_localize_script.
    public $js_file_name = 'critical-path-location.js'; // should be full file name plus extension
    ...
    public function __construct()
    {
        ...
        $this->title = __('Critical Path with location filter', 'disciple_tools');
    ...
    public function scripts()
    {
        ...
        wp_enqueue_script('dt_metrics_project_script', plugin_dir_url('') . 'disciple-tools-plugin-critical-path-location/critical-path/' . $this->js_file_name, [
            'moment',
            'jquery',
            'jquery-ui-core',
            'amcharts-core',
    ...
    public function data()
    {
        ...
        return [
            'cp' => self::critical_path_activity_location(dt_date_start_of_year(), time())
        ];
    }
    ...
    public function add_api_routes()
    {
        ...
        register_rest_route(
            $namespace, '/metrics/critical_path_activity_location', [
                [
                    'methods' => WP_REST_Server::READABLE,
    ...
    public function critical_path_activity_callback(WP_REST_Request $request)
    {
        ...
        if (isset($params["start"], $params["end"])) {
            ...
            $result = self::critical_path_activity_location($start, $end, $location);
            if (is_wp_error($result)) {
                return $result;
            } else {
                return new WP_REST_Response($result);
    ...
    /**
     * get the count for the different section like assigned, active contact to show in the criticla path Follow Up section
     * @param int $start
     * @param int $end
     * @param int $location // grid id for the selected location
     * @return array
     */
    public static function critical_path_activity_location($start = 0, $end = 0, $location = 0)
    {

        $data = [];
        $manual_additions = Disciple_Tools_Query::get_monthly_reports_count($start, $end);
        ...
        $new_contacts = Disciple_Tools_Query::new_contact_count($start, $end, $location);
        $current_contacts = Disciple_Tools_Query::new_contact_count(0, $end, $location);
        $data[] = [
            "key" => "new_contacts",
        ...
        $status_at_date = Disciple_Tools_Query::overall_status_at_date($end);
        $assigned_contacts = Disciple_Tools_Query::assigned_contacts_count($start, $end, $location);
        $data[] = [
        ...
        $active_contacts = Disciple_Tools_Query::active_contacts_count($start, $end, $location);
        $data[] = [
            "key" => "active_contacts",
        ...
        $seeker_path_counts = Disciple_Tools_Query::seeker_path_at_date($end);
        $seeker_path_activity = Disciple_Tools_Query::seeker_path_activity($start, $end);
        ...
        $baptisms = Disciple_Tools_Query::get_number_of_baptisms($start, $end, $location);
        $baptisms_total = Disciple_Tools_Query::get_number_of_baptisms(0, $end, $location);
        $data[] = [
            "key" => "baptisms",
        ...
        $baptism_generations = Disciple_Tools_Query::get_baptism_generations($start, $end, $location);
        $baptism_generations_total = Disciple_Tools_Query::get_baptism_generations(0, $end, $location);
        ...
        $baptizers = Disciple_Tools_Query::get_number_of_baptizers($start, $end);
        $total_baptizers = Disciple_Tools_Query::get_number_of_baptizers(0, $end);
        $data[] = [
            "key" => "baptizers",
        ...
        $active_groups = Disciple_Tools_Query::get_groups_count('active_groups', $start, $end);
        $current_groups = Disciple_Tools_Query::get_groups_count('active_groups', $end - 1, $end);
        $data[] = [
            "key" => "active_groups",
        ...
        $active_churches = Disciple_Tools_Query::get_groups_count('active_churches', $start, $end);
        $current_churches = Disciple_Tools_Query::get_groups_count('active_churches', $end - 1, $end);
        $data[] = [
            "key" => "active_churches",
...
//        @todo churches + groups
        $church_generations = Disciple_Tools_Query::get_groups_count('church_generations', $start, $end);
        $current_church_generations = Disciple_Tools_Query::get_groups_count('church_generations', $end - 1, $end);
        ...
        $church_planters = Disciple_Tools_Query::get_groups_count('church_planters', $start, $end);
        $total_church_planters = Disciple_Tools_Query::get_groups_count('church_planters', 0, $end);
        $data[] = [
            "key" => "church_planters",
...
new DT_Metrics_Critical_Path_Charts_location();
